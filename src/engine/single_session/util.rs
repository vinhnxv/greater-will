...
// ---------------------------------------------------------------------------
// Checkpoint preservation (protect against Rune Stop hook cleanup)
// ---------------------------------------------------------------------------

/// Preserve the current checkpoint before killing a session.
///
/// Rune's Stop hook (`on-session-stop.sh`) cancels `in_progress` phases in
/// the checkpoint when a session exits. This is destructive for GW's crash
/// recovery: the checkpoint data needed for `--resume` gets corrupted.
///
/// This function copies `checkpoint.json` to `checkpoint.json.gw-preserve`
/// BEFORE the kill. After restart, `restore_checkpoint_if_corrupted()` can
/// detect and undo the damage.
pub(crate) fn preserve_checkpoint(working_dir: &Path) {
    let loop_state = crate::monitor::loop_state::read_arc_loop_state(working_dir);
    let cp_path = match loop_state.active() {
        Some(state) => state.resolve_checkpoint_path(working_dir),
        None => return, // No active loop state — nothing to preserve
    };
    ...
    let preserve_path = cp_path.with_extension("json.gw-preserve");
    match std::fs::copy(&cp_path, &preserve_path) {
        Ok(_) => {
            info!(
                checkpoint = %cp_path.display(),
                preserve = %preserve_path.display(),
                "Checkpoint preserved before session kill"
            );
        }
        Err(e) => {
            warn!(error = %e, "Failed to preserve checkpoint (non-fatal)");
        }
    }
}
...
///
/// Detects corruption by checking if phases that were `in_progress` in the
/// preserved copy are now `cancelled` in the current checkpoint. If so,
/// restores the preserved copy.
///
/// Called before each crash-restart attempt in the orchestrator.
pub(crate) fn restore_checkpoint_if_corrupted(working_dir: &Path) {
    let loop_state = crate::monitor::loop_state::read_arc_loop_state(working_dir);
    let cp_path = match loop_state.active() {
        Some(state) => state.resolve_checkpoint_path(working_dir),
        None => return,
    ...
    let preserve_path = cp_path.with_extension("json.gw-preserve");
    if !preserve_path.exists() || !cp_path.exists() {
        return;
    }

    // Read both checkpoints
    let current = match crate::checkpoint::reader::read_checkpoint(&cp_path) {
        Ok(cp) => cp,
        Err(_) => return,
    };
    let preserved = match crate::checkpoint::reader::read_checkpoint(&preserve_path) {
        Ok(cp) => cp,
        Err(_) => {
            // Preserved copy is corrupt — remove it
            let _ = std::fs::remove_file(&preserve_path);
            return;
        }
    };

    // Check if any phase was in_progress in preserved but cancelled in current
    let mut corrupted = false;
    for (name, preserved_phase) in &preserved.phases {
        if preserved_phase.status == "in_progress" {
            if let Some(current_phase) = current.phases.get(name) {
                if current_phase.status == "cancelled" {
                    info!(
                        phase = name,
                        "Detected Rune Stop hook corruption: '{}' was in_progress → cancelled",
                        name,
                    );
                    corrupted = true;
                    break;
    ...
    if corrupted {
        // Restore from preserved copy
        match std::fs::copy(&preserve_path, &cp_path) {
            Ok(_) => {
                info!(
                    checkpoint = %cp_path.display(),
                    "Checkpoint restored from gw-preserve (undid Rune Stop hook damage)"
                );
                println!(
                    "[gw] Restored checkpoint (Rune Stop hook had cancelled in_progress phases)"
                );
            }
            Err(e) => {
                warn!(error = %e, "Failed to restore checkpoint from gw-preserve");
            }
        }
    }

    // Clean up preserved copy
    let _ = std::fs::remove_file(&preserve_path);
}
